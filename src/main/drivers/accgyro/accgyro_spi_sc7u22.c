...
// SC7U22 data is MSB-first (big-endian). Macro to assemble int16_t from
// two consecutive rxBuf bytes: high byte first.
#define SC7U22_INT16_FROM_MSB(buf, idx) ((int16_t)(((uint16_t)(buf)[(idx)] << 8) | (buf)[(idx) + 1]))

#ifdef USE_GYRO_EXTI
// Called in ISR context
// Gyro read has just completed
busStatus_e sc7u22Intcallback(uint32_t arg)
{
...
bool sc7u22AccRead(accDev_t *acc)
{
    ...
    {
        acc->gyro->dev.txBuf[0] = SC7U22_REG_ACC_XH | 0x80;
    ...
    case GYRO_EXTI_INT_DMA:
    {
        ...
        // SC7U22 data is MSB-first: rxBuf[1]=XH, rxBuf[2]=XL, rxBuf[3]=YH, rxBuf[4]=YL, rxBuf[5]=ZH, rxBuf[6]=ZL
        uint8_t *rxBuf = acc->gyro->dev.rxBuf;
        acc->ADCRaw[X] = SC7U22_INT16_FROM_MSB(rxBuf, 1);
        acc->ADCRaw[Y] = SC7U22_INT16_FROM_MSB(rxBuf, 3);
        acc->ADCRaw[Z] = SC7U22_INT16_FROM_MSB(rxBuf, 5);
        break;
    }
...
bool sc7u22GyroRead(gyroDev_t *gyro)
{
    ...
    case GYRO_EXTI_INIT:
    {
        ...
        if (gyro->detectedEXTI > GYRO_EXTI_DETECT_THRESHOLD) {
            if (spiUseDMA(&gyro->dev)) {
                ...
                gyro->dev.txBuf[0] = SC7U22_REG_ACC_XH | 0x80;
                gyro->segments[0].len = 14;
                gyro->segments[0].callback = sc7u22Intcallback;
    ...
    case GYRO_EXTI_NO_INT:
    {
        ...
        gyro->dev.txBuf[0] = SC7U22_REG_GYR_XH | 0x80;
        ...
        gyro->gyroADCRaw[X] = SC7U22_INT16_FROM_MSB(rxBuf, 1);
        gyro->gyroADCRaw[Y] = SC7U22_INT16_FROM_MSB(rxBuf, 3);
        gyro->gyroADCRaw[Z] = SC7U22_INT16_FROM_MSB(rxBuf, 5);
        break;
    }
    ...
    case GYRO_EXTI_INT_DMA:
    {
        ...
        uint8_t *rxBuf = gyro->dev.rxBuf;
        gyro->gyroADCRaw[X] = SC7U22_INT16_FROM_MSB(rxBuf, 7);
        gyro->gyroADCRaw[Y] = SC7U22_INT16_FROM_MSB(rxBuf, 9);
        gyro->gyroADCRaw[Z] = SC7U22_INT16_FROM_MSB(rxBuf, 11);
        break;
    }
