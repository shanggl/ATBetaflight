...
// SC7I22 registers (Bosch-family 6-axis IMU, SPI only)
// WHO_AM_I = 0x6A at reg 0x01. Data is MSB-first (big-endian).
// CPOL=1, CPHA=1 (SPI mode 3), max 10 MHz SPC clock.
typedef enum {
    SC7I22_REG_WHO_AM_I  = 0x01,
    SC7I22_REG_COM_CFG   = 0x05, // default 0x50 — BDU=1, Addr_Auto=1, 4-wire SPI
    SC7I22_REG_ACC_XH    = 0x0C, // ACC X high byte (MSB-first)
    SC7I22_REG_GYR_XH    = 0x12, // GYR X high byte (MSB-first)
    SC7I22_REG_TEMP_H    = 0x22, // Temperature MSB
    SC7I22_REG_ACC_CONF  = 0x40,
    SC7I22_REG_ACC_RANGE = 0x41,
    SC7I22_REG_GYR_CONF  = 0x42,
    SC7I22_REG_GYR_RANGE = 0x43,
    SC7I22_REG_SOFT_RST  = 0x4A,
    SC7I22_REG_PWR_CTRL  = 0x7D,
} sc7i22Register_e;

// Contained in accgyro_spi_sc7i22_init.c which is size optimized
uint8_t sc7i22Detect(const extDevice_t *dev);
bool sc7i22SpiAccDetect(accDev_t *acc);
bool sc7i22SpiGyroDetect(gyroDev_t *gyro);

// Contained in accgyro_spi_sc7i22.c which is speed optimized
